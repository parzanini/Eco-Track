...
onMounted( async () => {
  ...
  // Api Call
  const response = await axios.get(`http://localhost/CI4-EcoTrack/public/getLeaderBoard`, {
...
<!--    Leader board, table display -->
    <div class="container flex flex-col leaderboard h-[20em] overflow-y-scroll">
      ...
      <div v-if="isLoading" class="flex flex-row justify-center align-middle pt-10">
        <h1 class="mt-2 mr-4 text-2xl">Loading...</h1>
        <div class="w-12 h-12 border-4 border-gray-300 border-t-4 border-t-green-500 rounded-full animate-spin"></div>
      </div>
<!--      table of members, limit to 50 best -->
      <table  v-if="!isLoading" class="mb-10 mx-1 text-2xl">
        <thead>
        <tr>
          <th>Rank</th>
          <th>Current Badge</th>
...
<!--        using boolean to display leaderboard. Only show when populated-->
        <tbody v-for="(user, index) in users" :key="user.user_id">
        <tr>
          <td class="text-center">{{ index + 1}}</td>
          <td class="flex justify-center my-1">
<!--            set the badge based on the wasteLogScore-->
            <img v-if="user.wasteLogScore > 15" src="/imgs/badges/cil_badgegold-thumbnail.png" alt="gold badge" class="badgeThumbnail">
            <img v-if="user.wasteLogScore < 15 && user.wasteLogScore > 10 " src="/imgs/badges/cil_badgesilver-thumbnail.png" alt="silver badge" class="badgeThumbnail">
            <img v-if="user.wasteLogScore < 10 " src="/imgs/badges/cil_badgebronze-thumbnail.png" alt="bronze badge" class="badgeThumbnail">
          </td>
